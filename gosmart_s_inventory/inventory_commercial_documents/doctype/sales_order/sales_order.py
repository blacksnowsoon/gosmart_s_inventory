	...
	def update_bin_quantities(self, multiplier):
		...
		for item in self.items:
			if not item.warehouse:
				frappe.throw(_(f"Warehouse is required for item {item.item_code} in Sales Order {self.name}"))
			...
			if multiplier == 1:
				bin_info = frappe.get_doc("Bin", {"item_code": item.item_code, "warehouse": item.warehouse})
				...
				if available_qty < flt(item.qty):
					frappe.throw(_("Insufficient stock for item {0} in warehouse {1}. Available: {2}, Required: {3}").format(
						item.item_code, item.warehouse, available_qty, item.qty))
			reserved_qty_change = flt(item.qty) * multiplier
			frappe.db.sql("""
				UPDATE `tabBin`
				SET reserved_qty = reserved_qty + %s
				WHERE item_code = %s AND warehouse = %s
			""", (reserved_qty_change, item.item_code, item.warehouse))
